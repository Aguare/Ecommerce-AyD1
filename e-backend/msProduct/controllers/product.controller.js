...
productController.getProductsForCart = async (req, res) => {
	...
	try {
		...
		const { id } = req.params;
		const queryMoney = `
           SELECT pr.id, pr.name, pr.description, pr.price, primg.image_path, c.name as category FROM inventory i
			JOIN (
				SELECT FK_Product, FK_Branch, MAX(created_at) AS MaxDate
				FROM inventory
				GROUP BY FK_Branch, FK_Product
			) i2 ON i.FK_Branch = i2.FK_Branch AND i.FK_Product = i2.FK_Product AND i.created_at = i2.MaxDate
			JOIN product pr ON i.FK_Product = pr.id
			JOIN product_image primg ON pr.id = primg.FK_Product
			JOIN product_has_category phc ON pr.id = phc.FK_Product
			JOIN category c ON phc.FK_Category = c.id
			JOIN branch b ON i.FK_Branch = b.id
			WHERE i.FK_Branch = ? AND i.stock != 0
			GROUP BY i.FK_Product LIMIT 15;`;
